...
router.post('/bulk', authenticateToken, async (req, res, next) => {
  ...
  const BATCH_SIZE = 50;
  try {
    for (let start = 0; start < users.length; start += BATCH_SIZE) {
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          if (errs.length) {
            results.push({ index: globalIndex, error: errs.join(', ') });
            continue;
          }

          const savepoint = `sp${globalIndex}`;
          ...
          try {
            ...
            if (existing.recordset.length) {
              ...
              results.push({ index: globalIndex, id: ex.id, action: 'updated' });
              userId = ex.id;
            } else {
              ...
              results.push({ index: globalIndex, id: insertedId, action: 'inserted' });
              userId = insertedId;
            }
            ...
            results.push({ index: globalIndex, error: err.message });
          }
        }
        ...
        results.push({ batch: start / BATCH_SIZE, error: batchError.message });
      }
    }
